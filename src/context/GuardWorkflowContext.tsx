...
import { useGuardWorkflow, type GuardWorkflowState } from "@/hooks/useGuardWorkflow";
...
/**
 * Bugo Guard workflow context — parallel to WorkflowContext.tsx (main Bugo).
 * Polls shared /api/image-status (generic kie.ai job poller).
 * Auto-saves completed generations to /api/guard/gallery (NOT the main gallery).
 *
 * Reuses the shared usePolling and useNavigationGuard hooks — both are
 * stateless and brand-agnostic.
 */

interface GuardWorkflowContextType {
...
const GuardWorkflowContext = createContext<GuardWorkflowContextType | null>(null);
...
export function useGuardWorkflowContext() {
  const ctx = useContext(GuardWorkflowContext);
  if (!ctx) throw new Error("useGuardWorkflowContext must be used within GuardWorkflowProvider");
  return ctx;
}

export function GuardWorkflowProvider({ children }: { children: ReactNode }) {
  const workflow = useGuardWorkflow();
  ...
  const pollGenerations = useCallback(async () => {
    ...
    for (const { ref, gen } of active) {
      ...
      if (data.status !== gen.status || data.resultUrl) {
        ...
        // Auto-save to guard gallery (NOT the main gallery) when completed.
        if (data.status === "completed" && data.resultUrl) {
          ...
          (async () => {
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
              try {
                const saveRes = await fetch("/api/guard/gallery", {
                  method: "POST",
                ...
                console.warn(
                  `[guard save] attempt ${attempt + 1}/5 failed — ${lastError}`,
                  gen.jobId
                );
                ...
                console.warn(
                  `[guard save] attempt ${attempt + 1}/5 exception — ${lastError}`,
                  gen.jobId
                );
              }
            ...
            console.error(
              "Failed to save guard image after 5 attempts. Last error:",
              lastError,
              "JobId:",
              gen.jobId
            );
  ...
  return (
    <GuardWorkflowContext.Provider value={value}>
      {children}
    </GuardWorkflowContext.Provider>
  );
}
