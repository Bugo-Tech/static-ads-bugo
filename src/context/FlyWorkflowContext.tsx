...
/**
 * Bugo Fly workflow context — parallel to WorkflowContext.tsx (main Bugo).
 * Polls shared /api/image-status (generic kie.ai job poller).
 * Auto-saves completed generations to /api/fly/gallery (NOT the main gallery).
 *
 * Reuses the shared usePolling and useNavigationGuard hooks — both are
 * stateless and brand-agnostic.
 */

interface FlyWorkflowContextType {
  state: FlyWorkflowState;
...
const FlyWorkflowContext = createContext<FlyWorkflowContextType | null>(null);

export function useFlyWorkflowContext() {
  const ctx = useContext(FlyWorkflowContext);
  if (!ctx) throw new Error("useFlyWorkflowContext must be used within FlyWorkflowProvider");
  return ctx;
}
