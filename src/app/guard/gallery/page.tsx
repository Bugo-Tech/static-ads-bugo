...
/**
 * Bugo Guard Gallery — full 1:1 feature parity with src/app/gallery/page.tsx.
 * Type swapped (GuardGalleryImage), endpoints swapped (/api/guard/gallery,
 * /api/guard/generate-image), header text changed. Same colors (primary),
 * same buttons, same language detection, same parent-child grouping,
 * same RegenerateModal.
 */

import { useState, useEffect, useRef, DragEvent } from "react";
import Link from "next/link";
import type { GuardGalleryImage, GuardGalleryFolder } from "@/lib/guard-gallery";
...
export default function GuardGalleryPage() {
  const [images, setImages] = useState<GuardGalleryImage[]>([]);
  const [folders, setFolders] = useState<GuardGalleryFolder[]>([]);
  const [activeFolder, setActiveFolder] = useState<string>("root");
  const [draggedImageId, setDraggedImageId] = useState<string | null>(null);
  const [lightboxImage, setLightboxImage] = useState<GuardGalleryImage | null>(null);
  const [dragOverFolderId, setDragOverFolderId] = useState<string | null>(null);
  const [newFolderName, setNewFolderName] = useState("");
  const [showNewFolder, setShowNewFolder] = useState(false);
  const [selectedImages, setSelectedImages] = useState<Set<string>>(new Set());
  const [regenerateModal, setRegenerateModal] = useState<{ image: GuardGalleryImage; mode: "fix" | "cross-size" } | null>(null);
  ...
  async function loadGallery() {
    const res = await fetch("/api/guard/gallery");
    const data = await res.json();
    setImages(
      (data.images || []).sort(
        (a: GuardGalleryImage, b: GuardGalleryImage) =>
  ...
  async function handleDelete(id: string) {
    await fetch(`/api/guard/gallery?imageId=${id}`, { method: "DELETE" });
  ...
  async function handleDeleteSelected() {
    await Promise.all(
      [...selectedImages].map((id) =>
        fetch(`/api/guard/gallery?imageId=${id}`, { method: "DELETE" })
      )
    );
  ...
  async function handleDeleteFolder(id: string) {
    await fetch(`/api/guard/gallery?folderId=${id}`, { method: "DELETE" });
  ...
  function imageHasMetadata(img: GuardGalleryImage) {
    return !!(img.originalPrompt || (img.prompt && img.prompt.length > 10));
  }

  async function handleRegenerate(image: GuardGalleryImage, params: { fixInstruction?: string; targetSize: string }) {
...
${basePrompt}`;
  ...
  async function handleDownloadSelected() {
    const selected = [...selectedImages]
      ...
      .filter((img): img is GuardGalleryImage => !!img);
      ...
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <Link href="/guard" className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600">
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </Link>
            <h1 className="text-lg font-bold text-gray-900">Bugo Guard — Gallery</h1>
          ...
          <div className="flex items-center gap-2">
            {selectedImages.size > 0 && (
              <>
                ...
                <button onClick={() => setBatchFixModal(true)} className="rounded-lg bg-violet-50 px-3 py-1.5 text-sm font-medium text-violet-700 hover:bg-violet-100">
                  Fix Selected
                </button>
          ...
          <div className="flex-1">
            ...
            {filteredImages.length === 0 ? (
              <div className="rounded-2xl border-2 border-dashed border-border py-16 text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">Generated Bugo Guard ads will appear here automatically</p>
              </div>
            ) : (
              <GroupedGalleryGrid
                ...
                onFix={(img: GuardGalleryImage) => setRegenerateModal({ image: img, mode: "fix" })}
                onCrossSize={(img: GuardGalleryImage) => setRegenerateModal({ image: img, mode: "cross-size" })}
              ...
              {imageHasMetadata(lightboxImage) && (
                <>
                  <button
                    ...
                    className="rounded-lg bg-violet-500 px-4 py-2 text-sm font-medium text-white hover:bg-violet-600 transition-colors"
                  >
                    Fix Text
                  </button>
                  <button
...
interface GroupedGridProps {
  images: GuardGalleryImage[];
  selectedImages: Set<string>;
  pendingJobs: PendingJob[];
  onToggleSelect: (id: string) => void;
  onLightbox: (img: GuardGalleryImage) => void;
  onDelete: (id: string) => void;
  onDownload: (url: string, filename: string) => void;
  onDragStart: (id: string) => void;
  onFix: (img: GuardGalleryImage) => void;
  onCrossSize: (img: GuardGalleryImage) => void;
  imageHasMetadata: (img: GuardGalleryImage) => boolean;
}

function GroupedGalleryGrid(props: GroupedGridProps) {
  const { images } = props;

  const groups = new Map<string, GuardGalleryImage[]>();
...
function ReferenceGroup(props: GroupedGridProps & { refUrl: string; images: GuardGalleryImage[] }) {
  ...
  const byAngle = new Map<string, GuardGalleryImage[]>();
  for (const img of images) {
                    ...
                    <div key={img.id} className="flex gap-2">
                      ...
                      {imgChildren.map((child) => (
                        <div key={child.id} className="relative">
                          {child.isQcFix && (
                            <span className="absolute -top-1.5 -left-1.5 z-10 rounded-full bg-violet-500 px-1.5 py-0.5 text-[9px] font-bold text-white">🔧 QC</span>
                          )}
...
function detectImageLanguage(img: GuardGalleryImage): { flag: string; code: string } | null {
  const text = [
...
function GalleryImageCard({ img, selectedImages, pendingJobs, onToggleSelect, onLightbox, onDelete, onDownload, onDragStart, onFix, onCrossSize, imageHasMetadata }: Omit<GroupedGridProps, "images"> & { img: GuardGalleryImage }) {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-0.5">
            {imageHasMetadata(img) && (
              <>
                <button onClick={() => onFix(img)} className="rounded p-0.5 text-gray-400 hover:bg-violet-50 hover:text-violet-600" title="Fix">
