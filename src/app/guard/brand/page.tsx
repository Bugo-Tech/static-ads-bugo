...
import type { GuardBrandConfig, GuardProductImage } from "@/lib/guard-defaults";

export default function GuardBrandSettingsPage() {
  const [config, setConfig] = useState<GuardBrandConfig | null>(null);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    fetch("/api/guard/brand")
      .then((r) => r.json())
  ...
  function updateField<K extends keyof GuardBrandConfig>(key: K, value: GuardBrandConfig[K]) {
    setConfig((prev) => (prev ? { ...prev, [key]: value } : prev));
  }

  function updateSpec(key: keyof GuardBrandConfig["productSpecs"], value: string) {
    setConfig((prev) =>
      prev ? { ...prev, productSpecs: { ...prev.productSpecs, [key]: value } } : prev
    );
  }

  function updatePricing(key: keyof GuardBrandConfig["pricing"], value: string) {
    setConfig((prev) =>
      prev ? { ...prev, pricing: { ...prev.pricing, [key]: value } } : prev
    );
  }

  function updatePricingUS(key: keyof GuardBrandConfig["pricingUS"], value: string) {
    setConfig((prev) =>
  ...
  return (
    <div className="min-h-screen bg-surface">
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900">Bugo Guard — Brand Settings</h1>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-xl bg-violet-600 px-5 py-2 text-sm font-bold text-white shadow-sm hover:bg-violet-700 disabled:opacity-50 transition-all"
          >
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-6 py-8">
        {/* Product Images — single image expected for Bugo Guard. */}
        <Section
          title="Product Image"
          description="Upload the Bugo Guard device image. For best results upload ONE clean product photo; you can upload multiple if you want variants."
        >
          <GuardProductImageManager />
        </Section>
        ...
        <div className="flex justify-end pb-8">
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-xl bg-violet-600 px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-violet-700 disabled:opacity-50 transition-all"
          >
...
function GuardProductImageManager() {
  const [products, setProducts] = useState<GuardProductImage[]>([]);
  ...
  async function handleUpload(file: File, label: string) {
    ...
    try {
      const res = await fetch("/api/guard/products", { method: "POST", body: fd });
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await fetch(`/api/guard/products?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border-2 border-dashed border-violet-200 bg-violet-50/30 p-4">
        <div className="mb-2">
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-violet-600 focus:outline-none"
          />
        </div>
        <label className="flex cursor-pointer items-center gap-3 rounded-lg bg-white px-4 py-3 ring-1 ring-violet-300 hover:bg-violet-50">
          <input
            type="file"
            accept="image/*"
            className="hidden"
          ...
          <span className="text-sm font-medium text-violet-700">
            {uploading ? "Uploading..." : "Upload a Bugo Guard product image"}
          </span>
        </label>
      </div>
...
function PdfUploader({ market, onUploaded }: { market: "il" | "us"; onUploaded: (text: string) => void }) {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      const res = await fetch("/api/guard/brand/upload-pdf", { method: "POST", body: fd });
      const data = await res.json();
      if (res.ok) {
        setResult({ pages: data.pages, chars: data.chars });
        const configRes = await fetch("/api/guard/brand");
  ...
  return (
    <div className="mb-3">
      <label className="flex items-center gap-3 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 px-4 py-3 cursor-pointer hover:border-violet-400 hover:bg-violet-50 transition-colors">
        <input
          type="file"
          accept=".pdf"
          className="hidden"
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full rounded-lg border border-border px-3 py-2 text-sm text-gray-800 focus:border-violet-600 focus:outline-none focus:ring-1 focus:ring-violet-600"
      />
    </div>
  );
}
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-border px-3 py-1.5 text-sm text-gray-800 focus:border-violet-600 focus:outline-none focus:ring-1 focus:ring-violet-600"
          />
          <button
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={placeholder}
          className="flex-1 rounded-lg border border-dashed border-border px-3 py-1.5 text-sm text-gray-600 focus:border-violet-600 focus:outline-none focus:ring-1 focus:ring-violet-600"
        />
        <button
          onClick={addItem}
