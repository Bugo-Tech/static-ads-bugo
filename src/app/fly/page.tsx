...
import { useFlyWorkflowContext } from "@/context/FlyWorkflowContext";
...
export default function FlyHome() {
  ...
  const {
    ...
    reset,
    confirmNavigation,
  } = useFlyWorkflowContext();

  const [productImages, setProductImages] = useState<FlyProductImage[]>([]);
  ...
  async function handleAnalyze() {
    ...
    const promises = state.references.map(async (ref) => {
      ...
      try {
        ...
        const res = await fetch("/api/fly/analyze", {
          method: "POST",
          body: analyzeForm,
        });
      ...
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-600 text-sm font-bold text-white">
              🦟
            </div>
            <div>
              <h1 className="text-lg font-bold text-gray-900">Bugo Fly Ad Generator</h1>
              <p className="text-xs text-gray-500">Bugo Fly — mosquito and fly repeller</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
            ...
            <button
              onClick={() => router.push("/fly/gallery")}
              className="rounded-lg border border-border px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors"
            >
              Bugo Fly Gallery
            </button>
            <button
              onClick={() => router.push("/fly/brand")}
              className="rounded-lg border border-border px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors"
            >
              Bugo Fly Brand
            </button>
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={s.key} className="flex items-center gap-2">
                  {i > 0 && (
                    <div className={`h-px w-8 ${isPast ? "bg-cyan-600" : "bg-border"}`} />
                  )}
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className={`text-sm font-medium ${
                        isActive ? "text-gray-900" : isPast ? "text-cyan-700" : "text-gray-400"
                      }`}
                    >
                      {s.label}
                    </span>
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Upload competitor mosquito/fly-repellent ads as references.
                </p>
              </div>
              ...
              <>
                ...
                <div className="rounded-xl border border-border bg-white p-4">
                  <h3 className="mb-2 text-sm font-bold text-gray-700">
                    Bugo Fly product image (used in all references)
                  </h3>
                  {!productsLoading && productImages.length === 0 && (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                      <p className="text-sm text-amber-800">
                        ⚠️ No Bugo Fly product images uploaded yet.{" "}
                        <button
                          onClick={() => router.push("/fly/brand")}
                      ...
                      {productImages.map((prod) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {selected && (
                              <span className="text-xs font-bold text-cyan-700">✓ in use</span>
                            )}
                          </button>
                        );
                      })}
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={!canAnalyze}
                    className="rounded-xl bg-cyan-600 px-6 py-3 text-sm font-bold text-white shadow-sm transition-all hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    ...
                    {ref.analysis && (
                      <div className="mt-2">
                        <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${
                          (ref.analysis.niche as string) === "fly-mosquito"
                            ? "bg-cyan-100 text-cyan-700"
                            : "bg-blue-100 text-blue-700"
                        }`}>
                          {(ref.analysis.niche as string) === "fly-mosquito" ? "Same Niche" : "Cross-Niche"}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  className={`relative h-5 w-9 rounded-full transition-colors ${
                    state.enhancedVariationMatching ? "bg-cyan-600" : "bg-gray-300"
                  }`}
        ...
        {state.step === "generate" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-900">Generating Bugo Fly Ads</h2>
                <p className="text-sm text-gray-500">Your ads are being generated via Nano Banana</p>
              </div>
              {allDone && (
                <button
                  onClick={reset}
                  className="rounded-xl bg-cyan-600 px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-cyan-700"
                >
                  Start New Batch
                </button>
              )}
