...
/**
 * Bugo Fly Gallery — full 1:1 feature parity with src/app/gallery/page.tsx.
 * Type swapped (FlyGalleryImage), endpoints swapped (/api/fly/gallery,
 * /api/fly/generate-image), header text changed. Same colors (primary),
...
import type { FlyGalleryImage, FlyGalleryFolder } from "@/lib/fly-gallery";
...
export default function FlyGalleryPage() {
  const [images, setImages] = useState<FlyGalleryImage[]>([]);
  const [folders, setFolders] = useState<FlyGalleryFolder[]>([]);
  const [activeFolder, setActiveFolder] = useState<string>("root");
  const [draggedImageId, setDraggedImageId] = useState<string | null>(null);
  const [lightboxImage, setLightboxImage] = useState<FlyGalleryImage | null>(null);
  const [dragOverFolderId, setDragOverFolderId] = useState<string | null>(null);
  const [newFolderName, setNewFolderName] = useState("");
  const [showNewFolder, setShowNewFolder] = useState(false);
  const [selectedImages, setSelectedImages] = useState<Set<string>>(new Set());
  const [regenerateModal, setRegenerateModal] = useState<{ image: FlyGalleryImage; mode: "fix" | "cross-size" } | null>(null);
  ...
  async function loadGallery() {
    const res = await fetch("/api/fly/gallery");
    const data = await res.json();
    setImages(
      (data.images || []).sort(
        (a: FlyGalleryImage, b: FlyGalleryImage) =>
  ...
  async function handleDelete(id: string) {
    await fetch(`/api/fly/gallery?imageId=${id}`, { method: "DELETE" });
  ...
  async function handleDeleteSelected() {
    await Promise.all(
      [...selectedImages].map((id) =>
        fetch(`/api/fly/gallery?imageId=${id}`, { method: "DELETE" })
      )
    );
  ...
  async function handleDeleteFolder(id: string) {
    await fetch(`/api/fly/gallery?folderId=${id}`, { method: "DELETE" });
  ...
  function imageHasMetadata(img: FlyGalleryImage) {
    return !!(img.originalPrompt || (img.prompt && img.prompt.length > 10));
  }

  async function handleRegenerate(image: FlyGalleryImage, params: { fixInstruction?: string; targetSize: string }) {
...
${basePrompt}`;
    ...
    const res = await fetch("/api/fly/generate-image", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        prompt,
  ...
  async function handleDownloadSelected() {
    const selected = [...selectedImages]
      ...
      .filter((img): img is FlyGalleryImage => !!img);
      ...
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900">Bugo Fly — Gallery</h1>
          ...
          <div className="flex-1">
            ...
            {filteredImages.length === 0 ? (
              <div className="rounded-2xl border-2 border-dashed border-border py-16 text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">Generated Bugo Fly ads will appear here automatically</p>
              </div>
            ) : (
              <GroupedGalleryGrid
                ...
                onFix={(img: FlyGalleryImage) => setRegenerateModal({ image: img, mode: "fix" })}
                onCrossSize={(img: FlyGalleryImage) => setRegenerateModal({ image: img, mode: "cross-size" })}
...
interface GroupedGridProps {
  images: FlyGalleryImage[];
  selectedImages: Set<string>;
  pendingJobs: PendingJob[];
  onToggleSelect: (id: string) => void;
  onLightbox: (img: FlyGalleryImage) => void;
  onDelete: (id: string) => void;
  onDownload: (url: string, filename: string) => void;
  onDragStart: (id: string) => void;
  onFix: (img: FlyGalleryImage) => void;
  onCrossSize: (img: FlyGalleryImage) => void;
  imageHasMetadata: (img: FlyGalleryImage) => boolean;
}

function GroupedGalleryGrid(props: GroupedGridProps) {
  const { images } = props;

  const groups = new Map<string, FlyGalleryImage[]>();
...
function ReferenceGroup(props: GroupedGridProps & { refUrl: string; images: FlyGalleryImage[] }) {
  ...
  const byAngle = new Map<string, FlyGalleryImage[]>();
  for (const img of images) {
...
function detectImageLanguage(img: FlyGalleryImage): { flag: string; code: string } | null {
  const text = [
...
function GalleryImageCard({ img, selectedImages, pendingJobs, onToggleSelect, onLightbox, onDelete, onDownload, onDragStart, onFix, onCrossSize, imageHasMetadata }: Omit<GroupedGridProps, "images"> & { img: FlyGalleryImage }) {
