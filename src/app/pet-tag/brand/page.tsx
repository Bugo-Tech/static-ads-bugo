...
import type { PetTagBrandConfig, PetTagProductImage } from "@/lib/pet-tag-defaults";

export default function PetTagBrandSettingsPage() {
  const [config, setConfig] = useState<PetTagBrandConfig | null>(null);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    fetch("/api/pet-tag/brand")
      .then((r) => r.json())
  ...
  function updateField<K extends keyof PetTagBrandConfig>(key: K, value: PetTagBrandConfig[K]) {
    setConfig((prev) => (prev ? { ...prev, [key]: value } : prev));
  }

  function updateSpec(key: keyof PetTagBrandConfig["productSpecs"], value: string) {
    setConfig((prev) =>
      prev
  ...
  function updatePricing(key: keyof PetTagBrandConfig["pricing"], value: string) {
    setConfig((prev) =>
      prev
  ...
  function updatePricingUS(key: keyof PetTagBrandConfig["pricingUS"], value: string) {
    setConfig((prev) =>
      prev
        ? { ...prev, pricingUS: { ...prev.pricingUS, [key]: value } }
        : prev
    );
  }
  ...
  return (
    <div className="min-h-screen bg-surface">
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900">Bugo Pet Tag — Brand Settings</h1>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-xl bg-emerald-600 px-5 py-2 text-sm font-bold text-white shadow-sm hover:bg-emerald-700 disabled:opacity-50 transition-all"
          >
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-6 py-8">
        {/* Product Images — KEY for the pet-tag flow (2 images: product + packaging) */}
        <Section
          title="Product Images"
          description="Upload the Bugo Pet Tag pendant images. Recommended: one of the product itself, one of the packaging. Per reference ad, you'll choose which one to use."
        >
          <PetTagProductImageManager />
        </Section>
        ...
        <Section title="Brand Book — Israel / Hebrew" description="Upload a PDF — text will be extracted and used as context for ad copy generation.">
        ...
        <Section title="Brand Book — US / English" description="Upload a PDF — text will be extracted and used as context for English/foreign-market ads.">
        ...
        <div className="flex justify-end pb-8">
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-xl bg-emerald-600 px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-emerald-700 disabled:opacity-50 transition-all"
          >
...
// --- Pet Tag Product Image Manager (2-slot uploader) ---
function PetTagProductImageManager() {
  const [products, setProducts] = useState<PetTagProductImage[]>([]);
  ...
  async function handleUpload(file: File, label: string) {
    ...
    try {
      const res = await fetch("/api/pet-tag/products", { method: "POST", body: fd });
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await fetch(`/api/pet-tag/products?id=${id}`, { method: "DELETE" });
    ...
    <div className="space-y-4">
      <div className="rounded-lg border-2 border-dashed border-emerald-200 bg-emerald-50/30 p-4">
        <div className="mb-2">
          <label className="block text-xs font-medium text-gray-600">
            Label (e.g., &quot;product&quot;, &quot;packaging&quot;, or anything you want)
          </label>
          <input
            type="text"
            value={labelInput}
            ...
            className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-emerald-600 focus:outline-none"
          />
        </div>
        <label className="flex cursor-pointer items-center gap-3 rounded-lg bg-white px-4 py-3 ring-1 ring-emerald-300 hover:bg-emerald-50">
          <input
            type="file"
            accept="image/*"
            className="hidden"
          ...
          <span className="text-sm font-medium text-emerald-700">
            {uploading ? "Uploading..." : "Upload a Bugo Pet Tag product image"}
          </span>
        </label>
      </div>
      ...
      {products.length === 0 && (
        <p className="text-sm text-gray-500 italic">
          No product images yet. Upload at least one (typically two: the pendant and its packaging).
        </p>
      )}
    </div>
  );
...
function PdfUploader({ market, onUploaded }: { market: "il" | "us"; onUploaded: (text: string) => void }) {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      const res = await fetch("/api/pet-tag/brand/upload-pdf", { method: "POST", body: fd });
      const data = await res.json();
      if (res.ok) {
        setResult({ pages: data.pages, chars: data.chars });
        const configRes = await fetch("/api/pet-tag/brand");
  ...
  return (
    <div className="mb-3">
      <label className="flex items-center gap-3 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 px-4 py-3 cursor-pointer hover:border-emerald-400 hover:bg-emerald-50 transition-colors">
        <input
          type="file"
          accept=".pdf"
          className="hidden"
      ...
      {result && (
        <p className="mt-1.5 text-xs text-green-600">
          Extracted {result.chars.toLocaleString()} characters from {result.pages} pages. Content loaded below.
        </p>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full rounded-lg border border-border px-3 py-2 text-sm text-gray-800 focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600"
      />
    </div>
  );
}
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-border px-3 py-1.5 text-sm text-gray-800 focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600"
          />
          <button
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={placeholder}
          className="flex-1 rounded-lg border border-dashed border-border px-3 py-1.5 text-sm text-gray-600 focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600"
        />
        <button
          onClick={addItem}
