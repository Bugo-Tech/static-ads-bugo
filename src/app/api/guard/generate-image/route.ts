...
import { getGuardProductRules } from "@/lib/guard-prompts";
import type { Language } from "@/lib/types";

const PRODUCT_SCOPE: ProductScope = "guard";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const brand = await loadGuardBrandConfig();
    let finalPrompt = prompt;

    let publicRefUrl: string | undefined;
    if (referenceImageUrl?.startsWith("/api/guard/products/file/")) {
    ...
    console.log("=== GUARD GENERATE IMAGE DEBUG ===");
...
This overrides any aspect-ratio mention inside the LAYOUT REFERENCE.`);
      ...
      if (publicProductUrl) {
        rules.push(...getGuardProductRules(brand, true));
      } else {
...
Illustrations and depicted subjects (mice, rodents, a protected kitchen cabinet, clean pantry, a calm home) must reinforce the variation "${copyVariation.angle}".`);
      }
    ...
    console.error("Guard generate image error:", error);
