...
import { analyzeGuardReference } from "@/lib/guard-claude";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      } else if (imageUrl.startsWith("/api/guard/products/file/")) {
    ...
    const result = await analyzeGuardReference(imageBase64!, mimeType!, language);
