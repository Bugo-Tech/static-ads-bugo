...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Guard gallery error:", message, stack);
    return NextResponse.json(
      { error: message, where: "POST /api/guard/gallery" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error("Guard gallery delete error:", error);
