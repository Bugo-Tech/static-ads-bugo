...
import { analyzeFlyReference } from "@/lib/fly-claude";
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (buffer.length > 4 * 1024 * 1024) {
        ...
        const tmpIn = `/tmp/fly-analyze-in-${Date.now()}.${mimeType.includes("png") ? "png" : "jpg"}`;
        const tmpOut = `/tmp/fly-analyze-out-${Date.now()}.jpg`;
      ...
      if (imageUrl.startsWith("/api/fly/products/file/")) {
        const filename = imageUrl.split("/").pop()!;
        const filepath = path.join(process.cwd(), "uploads", "fly-products", filename);
    ...
    const result = await analyzeFlyReference(imageBase64, mimeType, language);
    return NextResponse.json(result);
  } catch (error) {
    console.error("Fly analyze error:", error);
