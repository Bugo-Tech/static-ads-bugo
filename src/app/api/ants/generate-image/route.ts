...
import { loadAntsBrandConfig } from "@/lib/ants-claude";
import { getAntsProductRules } from "@/lib/ants-prompts";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const brand = await loadAntsBrandConfig();
    let finalPrompt = prompt;

    let publicRefUrl: string | undefined;
    if (referenceImageUrl?.startsWith("/api/ants/products/file/")) {
    ...
    if (!isCrossSize && includeProduct && Array.isArray(productImageIds) && productImageIds.length > 0) {
      try {
        const indexPath = path.join(process.cwd(), "uploads", "ants-products", "index.json");
        const indexData = JSON.parse(await readFile(indexPath, "utf-8"));
        const product = indexData.find((p: { id: string }) => p.id === productImageIds[0]);
        if (product) {
          const filepath = path.join(process.cwd(), "uploads", "ants-products", product.filename);
    ...
    console.log("=== ANTS GENERATE IMAGE DEBUG ===");
...
This overrides any aspect-ratio mention inside the LAYOUT REFERENCE.`);
      ...
      if (publicProductUrl) {
        rules.push(...getAntsProductRules(brand, true));
      } else {
...
Illustrations and depicted subjects (ants trailing on kitchen counter, spilled sugar, clean tile, mom relieved) must reinforce the variation "${copyVariation.angle}".`);
      }
    ...
    console.error("Ants generate image error:", error);
