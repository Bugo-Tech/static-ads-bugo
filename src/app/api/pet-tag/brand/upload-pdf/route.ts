...
export async function POST(request: NextRequest) {
  try {
    ...
    let config: PetTagBrandConfig;
    try {
      const data = await readFile(CONFIG_FILE, "utf-8");
      config = { ...defaultPetTagBrandConfig, ...JSON.parse(data) };
    } catch {
      config = { ...defaultPetTagBrandConfig };
    }
    ...
    console.error("Pet Tag PDF upload error:", error);
    return NextResponse.json(
