...
import type { AntsBrandConfig, AntsProductImage } from "@/lib/ants-defaults";

export default function AntsBrandSettingsPage() {
  const [config, setConfig] = useState<AntsBrandConfig | null>(null);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    fetch("/api/ants/brand")
      .then((r) => r.json())
  ...
  function updateField<K extends keyof AntsBrandConfig>(key: K, value: AntsBrandConfig[K]) {
    setConfig((prev) => (prev ? { ...prev, [key]: value } : prev));
  }

  function updateSpec(key: keyof AntsBrandConfig["productSpecs"], value: string) {
    setConfig((prev) =>
      prev ? { ...prev, productSpecs: { ...prev.productSpecs, [key]: value } } : prev
    );
  }

  function updatePricing(key: keyof AntsBrandConfig["pricing"], value: string) {
    setConfig((prev) =>
      prev ? { ...prev, pricing: { ...prev.pricing, [key]: value } } : prev
    );
  }

  function updatePricingUS(key: keyof AntsBrandConfig["pricingUS"], value: string) {
    setConfig((prev) =>
  ...
  return (
    <div className="min-h-screen bg-surface">
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <Link href="/ants" className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600">
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </Link>
            <h1 className="text-lg font-bold text-gray-900">Bugo Ants — Brand Settings</h1>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-xl bg-orange-600 px-5 py-2 text-sm font-bold text-white shadow-sm hover:bg-orange-700 disabled:opacity-50 transition-all"
          >
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-6 py-8">
        {/* Product Images — single image expected for Bugo Ants. */}
        <Section
          title="Product Image"
          description="Upload the Bugo Ants device image. For best results upload ONE clean product photo; you can upload multiple if you want variants."
        >
          <AntsProductImageManager />
        </Section>
        ...
        <div className="flex justify-end pb-8">
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-xl bg-orange-600 px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-orange-700 disabled:opacity-50 transition-all"
          >
...
function AntsProductImageManager() {
  const [products, setProducts] = useState<AntsProductImage[]>([]);
  ...
  async function handleUpload(file: File, label: string) {
    ...
    try {
      const res = await fetch("/api/ants/products", { method: "POST", body: fd });
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await fetch(`/api/ants/products?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border-2 border-dashed border-orange-200 bg-orange-50/30 p-4">
        <div className="mb-2">
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-orange-600 focus:outline-none"
          />
        </div>
        <label className="flex cursor-pointer items-center gap-3 rounded-lg bg-white px-4 py-3 ring-1 ring-orange-300 hover:bg-orange-50">
          <input
            type="file"
            accept="image/*"
            className="hidden"
          ...
          <span className="text-sm font-medium text-orange-700">
            {uploading ? "Uploading..." : "Upload a Bugo Ants product image"}
          </span>
        </label>
      </div>
...
function PdfUploader({ market, onUploaded }: { market: "il" | "us"; onUploaded: (text: string) => void }) {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      const res = await fetch("/api/ants/brand/upload-pdf", { method: "POST", body: fd });
      const data = await res.json();
      if (res.ok) {
        setResult({ pages: data.pages, chars: data.chars });
        const configRes = await fetch("/api/ants/brand");
  ...
  return (
    <div className="mb-3">
      <label className="flex items-center gap-3 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 px-4 py-3 cursor-pointer hover:border-orange-400 hover:bg-orange-50 transition-colors">
        <input
          type="file"
          accept=".pdf"
          className="hidden"
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full rounded-lg border border-border px-3 py-2 text-sm text-gray-800 focus:border-orange-600 focus:outline-none focus:ring-1 focus:ring-orange-600"
      />
    </div>
  );
}
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-border px-3 py-1.5 text-sm text-gray-800 focus:border-orange-600 focus:outline-none focus:ring-1 focus:ring-orange-600"
          />
          <button
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={placeholder}
          className="flex-1 rounded-lg border border-dashed border-border px-3 py-1.5 text-sm text-gray-600 focus:border-orange-600 focus:outline-none focus:ring-1 focus:ring-orange-600"
        />
        <button
          onClick={addItem}
