...
/**
 * Bugo Ants Gallery — full 1:1 feature parity with src/app/gallery/page.tsx.
 * Type swapped (AntsGalleryImage), endpoints swapped (/api/ants/gallery,
 * /api/ants/generate-image), header text changed. Same colors (primary),
...
import type { AntsGalleryImage, AntsGalleryFolder } from "@/lib/ants-gallery";
...
export default function AntsGalleryPage() {
  const [images, setImages] = useState<AntsGalleryImage[]>([]);
  const [folders, setFolders] = useState<AntsGalleryFolder[]>([]);
  const [activeFolder, setActiveFolder] = useState<string>("root");
  const [draggedImageId, setDraggedImageId] = useState<string | null>(null);
  const [lightboxImage, setLightboxImage] = useState<AntsGalleryImage | null>(null);
  const [dragOverFolderId, setDragOverFolderId] = useState<string | null>(null);
  const [newFolderName, setNewFolderName] = useState("");
  const [showNewFolder, setShowNewFolder] = useState(false);
  const [selectedImages, setSelectedImages] = useState<Set<string>>(new Set());
  const [regenerateModal, setRegenerateModal] = useState<{ image: AntsGalleryImage; mode: "fix" | "cross-size" } | null>(null);
  ...
  async function loadGallery() {
    const res = await fetch("/api/ants/gallery");
    const data = await res.json();
    setImages(
      (data.images || []).sort(
        (a: AntsGalleryImage, b: AntsGalleryImage) =>
  ...
  async function handleDelete(id: string) {
    await fetch(`/api/ants/gallery?imageId=${id}`, { method: "DELETE" });
  ...
  async function handleDeleteSelected() {
    await Promise.all(
      [...selectedImages].map((id) =>
        fetch(`/api/ants/gallery?imageId=${id}`, { method: "DELETE" })
      )
    );
  ...
  async function handleDeleteFolder(id: string) {
    await fetch(`/api/ants/gallery?folderId=${id}`, { method: "DELETE" });
  ...
  function imageHasMetadata(img: AntsGalleryImage) {
    return !!(img.originalPrompt || (img.prompt && img.prompt.length > 10));
  }

  async function handleRegenerate(image: AntsGalleryImage, params: { fixInstruction?: string; targetSize: string }) {
...
${basePrompt}`;
  ...
  async function handleDownloadSelected() {
    const selected = [...selectedImages]
      ...
      .filter((img): img is AntsGalleryImage => !!img);
      ...
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900">Bugo Ants — Gallery</h1>
          ...
          <div className="flex items-center gap-2">
            {selectedImages.size > 0 && (
              <>
                ...
                <button onClick={() => setBatchFixModal(true)} className="rounded-lg bg-orange-50 px-3 py-1.5 text-sm font-medium text-orange-700 hover:bg-orange-100">
                  Fix Selected
                </button>
          ...
          <div className="flex-1">
            ...
            {filteredImages.length === 0 ? (
              <div className="rounded-2xl border-2 border-dashed border-border py-16 text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">Generated Bugo Ants ads will appear here automatically</p>
              </div>
            ) : (
              <GroupedGalleryGrid
                ...
                onFix={(img: AntsGalleryImage) => setRegenerateModal({ image: img, mode: "fix" })}
                onCrossSize={(img: AntsGalleryImage) => setRegenerateModal({ image: img, mode: "cross-size" })}
...
interface GroupedGridProps {
  images: AntsGalleryImage[];
  selectedImages: Set<string>;
  pendingJobs: PendingJob[];
  onToggleSelect: (id: string) => void;
  onLightbox: (img: AntsGalleryImage) => void;
  onDelete: (id: string) => void;
  onDownload: (url: string, filename: string) => void;
  onDragStart: (id: string) => void;
  onFix: (img: AntsGalleryImage) => void;
  onCrossSize: (img: AntsGalleryImage) => void;
  imageHasMetadata: (img: AntsGalleryImage) => boolean;
}

function GroupedGalleryGrid(props: GroupedGridProps) {
  const { images } = props;

  const groups = new Map<string, AntsGalleryImage[]>();
...
function ReferenceGroup(props: GroupedGridProps & { refUrl: string; images: AntsGalleryImage[] }) {
  ...
  const byAngle = new Map<string, AntsGalleryImage[]>();
  for (const img of images) {
                    ...
                    <div key={img.id} className="flex gap-2">
                      ...
                      {imgChildren.map((child) => (
                        <div key={child.id} className="relative">
                          {child.isQcFix && (
                            <span className="absolute -top-1.5 -left-1.5 z-10 rounded-full bg-orange-500 px-1.5 py-0.5 text-[9px] font-bold text-white">🔧 QC</span>
                          )}
...
function detectImageLanguage(img: AntsGalleryImage): { flag: string; code: string } | null {
  const text = [
...
function GalleryImageCard({ img, selectedImages, pendingJobs, onToggleSelect, onLightbox, onDelete, onDownload, onDragStart, onFix, onCrossSize, imageHasMetadata }: Omit<GroupedGridProps, "images"> & { img: AntsGalleryImage }) {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-0.5">
            {imageHasMetadata(img) && (
              <>
                <button onClick={() => onFix(img)} className="rounded p-0.5 text-gray-400 hover:bg-orange-50 hover:text-orange-600" title="Fix">
