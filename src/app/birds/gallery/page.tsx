...
/**
 * Bugo Birds Gallery — full 1:1 feature parity with src/app/gallery/page.tsx.
 * Type swapped (BirdsGalleryImage), endpoints swapped (/api/birds/gallery,
 * /api/birds/generate-image), header text changed. Same colors (primary),
...
import type { BirdsGalleryImage, BirdsGalleryFolder } from "@/lib/birds-gallery";
...
export default function BirdsGalleryPage() {
  const [images, setImages] = useState<BirdsGalleryImage[]>([]);
  const [folders, setFolders] = useState<BirdsGalleryFolder[]>([]);
  const [activeFolder, setActiveFolder] = useState<string>("root");
  const [draggedImageId, setDraggedImageId] = useState<string | null>(null);
  const [lightboxImage, setLightboxImage] = useState<BirdsGalleryImage | null>(null);
  const [dragOverFolderId, setDragOverFolderId] = useState<string | null>(null);
  const [newFolderName, setNewFolderName] = useState("");
  const [showNewFolder, setShowNewFolder] = useState(false);
  const [selectedImages, setSelectedImages] = useState<Set<string>>(new Set());
  const [regenerateModal, setRegenerateModal] = useState<{ image: BirdsGalleryImage; mode: "fix" | "cross-size" } | null>(null);
  ...
  async function loadGallery() {
    const res = await fetch("/api/birds/gallery");
    const data = await res.json();
    setImages(
      (data.images || []).sort(
        (a: BirdsGalleryImage, b: BirdsGalleryImage) =>
  ...
  async function handleDelete(id: string) {
    await fetch(`/api/birds/gallery?imageId=${id}`, { method: "DELETE" });
  ...
  async function handleDeleteSelected() {
    await Promise.all(
      [...selectedImages].map((id) =>
        fetch(`/api/birds/gallery?imageId=${id}`, { method: "DELETE" })
      )
    );
  ...
  async function handleDeleteFolder(id: string) {
    await fetch(`/api/birds/gallery?folderId=${id}`, { method: "DELETE" });
  ...
  function imageHasMetadata(img: BirdsGalleryImage) {
    return !!(img.originalPrompt || (img.prompt && img.prompt.length > 10));
  }

  async function handleRegenerate(image: BirdsGalleryImage, params: { fixInstruction?: string; targetSize: string }) {
...
${basePrompt}`;
  ...
  async function handleDownloadSelected() {
    const selected = [...selectedImages]
      ...
      .filter((img): img is BirdsGalleryImage => !!img);
      ...
      <header className="border-b border-border bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900">Bugo Birds — Gallery</h1>
          ...
          <div className="flex-1">
            ...
            {filteredImages.length === 0 ? (
              <div className="rounded-2xl border-2 border-dashed border-border py-16 text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">Generated Bugo Birds ads will appear here automatically</p>
              </div>
            ) : (
              <GroupedGalleryGrid
                ...
                onFix={(img: BirdsGalleryImage) => setRegenerateModal({ image: img, mode: "fix" })}
                onCrossSize={(img: BirdsGalleryImage) => setRegenerateModal({ image: img, mode: "cross-size" })}
...
interface GroupedGridProps {
  ...
  onLightbox: (img: BirdsGalleryImage) => void;
  onDelete: (id: string) => void;
  onDownload: (url: string, filename: string) => void;
  onDragStart: (id: string) => void;
  onFix: (img: BirdsGalleryImage) => void;
  onCrossSize: (img: BirdsGalleryImage) => void;
  imageHasMetadata: (img: BirdsGalleryImage) => boolean;
}

function GroupedGalleryGrid(props: GroupedGridProps) {
  const { images } = props;

  const groups = new Map<string, BirdsGalleryImage[]>();
...
function ReferenceGroup(props: GroupedGridProps & { refUrl: string; images: BirdsGalleryImage[] }) {
  ...
  const byAngle = new Map<string, BirdsGalleryImage[]>();
  for (const img of images) {
...
function detectImageLanguage(img: BirdsGalleryImage): { flag: string; code: string } | null {
  const text = [
...
function GalleryImageCard({ img, selectedImages, pendingJobs, onToggleSelect, onLightbox, onDelete, onDownload, onDragStart, onFix, onCrossSize, imageHasMetadata }: Omit<GroupedGridProps, "images"> & { img: BirdsGalleryImage }) {
