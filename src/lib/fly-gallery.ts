/**
 * Bugo Fly gallery — parallel to pet-tag-gallery.ts. Writes to
 * uploads/fly-gallery/. Independent atomic-write + backup safety. No shared
...
const GALLERY_DIR = path.join(process.cwd(), "uploads", "fly-gallery");
...
export interface FlyGalleryImage {
  id: string;
  filename: string;
  url: string;
  sourceUrl: string;
...
export interface FlyGalleryFolder {
  id: string;
  name: string;
  createdAt: string;
}

interface FlyGalleryIndex {
  images: FlyGalleryImage[];
  folders: FlyGalleryFolder[];
}
...
async function readIndex(): Promise<FlyGalleryIndex> {
  try {
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.images)) {
      console.warn("Fly gallery index was corrupted — restored from backup");
      return parsed;
    }
...
async function writeIndexUnsafe(index: FlyGalleryIndex) {
  await ensureDir();
  if (!(await hasEnoughDiskSpace())) {
    throw new Error(
      "Disk space critically low — refusing to write fly gallery index. Free up space and try again."
    );
  }
  try {
...
async function writeIndex(index: FlyGalleryIndex) {
...
export async function getFlyGallery(): Promise<FlyGalleryIndex> {
  await ensureDir();
  return readIndex();
}

export async function addImageToFlyGallery(
  image: Omit<FlyGalleryImage, "id" | "filename" | "url" | "createdAt">
): Promise<FlyGalleryImage> {
  await ensureDir();
  const id = `fly-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  ...
  try {
    ...
    throw new Error(`addImageToFlyGallery: download/write failed — ${detail}`);
  }

  const galleryImage: FlyGalleryImage = {
    ...image,
    id,
    filename,
    url: `/api/fly/gallery/file/${filename}`,
...
export async function deleteFlyGalleryImage(id: string): Promise<void> {
  const index = await readIndex();
...
export async function moveImageToFlyFolder(imageId: string, folderId: string): Promise<void> {
  const index = await readIndex();
...
export async function createFlyFolder(name: string): Promise<FlyGalleryFolder> {
  const index = await readIndex();
  const folder: FlyGalleryFolder = {
...
export async function deleteFlyFolder(id: string): Promise<void> {
  const index = await readIndex();
  index.images = index.images.map((img) =>
...
export async function renameFlyFolder(id: string, name: string): Promise<void> {
  const index = await readIndex();
