/**
 * Bugo Fly prompts — adapted from pet-tag-prompts.ts but focused on mosquitoes
 * and flies. Tight, focused instructions; runtime rules in
 * src/app/api/fly/generate-image/route.ts handle the language consistency
 * and product-fidelity enforcement.
 */

import type { Language } from "./types";
import type { FlyBrandConfig } from "./fly-defaults";
...
export function getFlyAnalysisPrompt(brand: FlyBrandConfig, language: Language): string {
  ...
  return `You are a Visual Template Replicator and Ad Adaptation Expert for "${brand.productName}" — a mosquito/fly repeller device.
...
### Scenario A: Same Niche (mosquito/fly/insect repeller / electric bug zapper / citronella / insecticide)
...
If the reference is NOT mosquito/fly repellent, replicate the LAYOUT, COMPOSITION, TYPOGRAPHY STYLE, COLORS — but REPLACE niche-specific visuals with mosquito/fly-protection equivalents. ZERO traces of the original niche.
...
- Niche-specific background imagery → mosquito/fly-relevant equivalents (mosquito silhouettes, calm bedroom/balcony scenes, shield/protection imagery, sleeping baby/family at home, peaceful evening)
- Niche-specific icons → relevant icons (mosquito, fly, midge, "Z" sleep symbol, shield, checkmark)
- Before/after concepts → "before = mosquitoes biting / kids scratching → after = peaceful sleep / mosquito-free"
- Testimonials → parent/family/homeowner testimonials about peaceful sleep
...
## OUTPUT FORMAT (JSON)
{
  ...
  "copyVariations": [
    {
      "id": "var-1",
      "angle": "e.g., peaceful sleep without buzzing",
      "sections": [
        {
          "id": "s1",
          "label": "headline",
...
VARIATIONS 2-4: Different marketing angles (sleep / peaceful evening / kids' safety / no chemicals).
...
1. **Opening**:
   ...
   - Different niche: "REPLICATE layout / composition / typography / colors. ADAPT visuals to mosquito/fly protection. Brand: '${brand.productName}'. Text in ${lang}, ${dir === "RTL (right-to-left)" ? "right-aligned (RTL)" : "left-aligned (LTR)"}."
...
4. **PRODUCT**:
   - ${brand.productName} is an electronic mosquito/fly repeller device.
 ...
 */
export function getFlyProductRules(brand: FlyBrandConfig, hasProductImage: boolean): string[] {
  const rules: string[] = [];

  if (hasProductImage) {
    rules.push(`CRITICAL — ${brand.productName.toUpperCase()} IS A DEDICATED PRODUCT:
The brand name "${brand.productName}" contains the word "Bugo" — this is intentional and shares branding with other Bugo product lines. However, ${brand.productName} is a SPECIFIC PRODUCT model. Your ONLY visual source for what ${brand.productName} looks like is Image 2.
...
The reference image (Image 1) likely contains a competitor product such as "PestLab", "Bug Bomb", or another branded mosquito/fly device. Wherever that competitor product appears in Image 1:
...
2. The BRAND TEXT on the device ("PestLab", "PESTLAB", "Pest Lab", or any other competitor brand name printed on the product face) is COMPLETELY ERASED — it does not appear anywhere in your output. The only branding on the product face is Image 2's own built-in branding.
3. Any DEVICE-SPECIFIC visual features (LED ring color, button placement, screen, indicator lights, ventilation slots) come from IMAGE 2, not Image 1. Do not preserve features from Image 1's device.
...
- Rendering Image 2's shape but keeping the competitor's brand text ("PestLab" alongside "${brand.productName}")
- A HYBRID device that combines the competitor's body with Image 2's coloring or vice versa
- Two brand names on one product (e.g., "PestLab ${brand.productName}" or "PESTLAB" stacked with "${brand.productName}")
- ANY occurrence of the word "PestLab", "PESTLAB", "Pest Lab", or any competitor brand name anywhere in the output (on the product, in the background, or in text blocks)
...
- If Image 1 (the reference) contains a competitor mosquito/fly product (electric zapper, citronella candle, plug-in repeller, coil, spray bottle, sticker), REMOVE the competitor entirely (device + brand text + all its visual features per the COMPETITOR ERASURE rule above) and place Image 2 in that single position.
...
- Same shape (do NOT render it as a different device, alternative form factor, or generic "mosquito repeller" interpretation. If Image 2 is elongated, the output is elongated. If Image 2 is rounded, the output is rounded. If Image 2 is white with a blue stripe, the output is white with a blue stripe — not "similar", IDENTICAL.)
...
- Same details: LED color, button positions, decorative grooves, vents — all match Image 2
