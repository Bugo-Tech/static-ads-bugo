/**
 * Bugo Guard Claude wrapper — parallel to pet-tag-claude.ts. Uses guard brand
...
import { getGuardAnalysisPrompt } from "./guard-prompts";
import { defaultGuardBrandConfig, type GuardBrandConfig } from "./guard-defaults";
...
export async function loadGuardBrandConfig(): Promise<GuardBrandConfig> {
  return readBrandConfigFile("guard", defaultGuardBrandConfig);
}

export async function analyzeGuardReference(
  imageBase64: string,
  mimeType: string,
  language: Language
): Promise<{ analysis: AnalysisResult; copyVariations: CopyVariation[] }> {
  const client = getClient();
  const brandConfig = await loadGuardBrandConfig();

  const systemPrompt = getGuardAnalysisPrompt(brandConfig, language);
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: "Analyze this reference ad image and generate the Nano Banana prompt and copy variations adapted for the Bugo Guard essential-oil rodent repellent sachets. Return the response as JSON.",
          },
        ],
      },
    ],
...
export async function translateGuardVariations(
  ...
  if (Object.keys(inputMap).length === 0) {
    throw new Error(`Guard translator: no source text. direction=${direction}`);
  }

  const systemPrompt = `You are a professional marketing translator for Bugo Guard, essential-oil sachets that repel mice and rodents.

Translate ad copy from ${sourceLangName} to ${targetLangName}.

Style:
- Natural, idiomatic, marketing-grade ${targetLangName}. Speaks to homeowners who have found droppings, chewed food packaging or gnawed wires and do not want poison or traps in the house — tone is calm, practical, reassuring, focused on a chemical-free home that stays protected.
