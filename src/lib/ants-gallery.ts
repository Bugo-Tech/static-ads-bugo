/**
 * Bugo Ants gallery — parallel to pet-tag-gallery.ts. Writes to
 * uploads/ants-gallery/. Independent atomic-write + backup safety. No shared
...
const GALLERY_DIR = path.join(process.cwd(), "uploads", "ants-gallery");
...
export interface AntsGalleryImage {
  id: string;
  filename: string;
  url: string;
  sourceUrl: string;
...
export interface AntsGalleryFolder {
  id: string;
  name: string;
  createdAt: string;
}

interface AntsGalleryIndex {
  images: AntsGalleryImage[];
  folders: AntsGalleryFolder[];
}
...
async function readIndex(): Promise<AntsGalleryIndex> {
  try {
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.images)) {
      console.warn("Ants gallery index was corrupted — restored from backup");
      return parsed;
    }
...
async function writeIndexUnsafe(index: AntsGalleryIndex) {
  await ensureDir();
  if (!(await hasEnoughDiskSpace())) {
    throw new Error(
      "Disk space critically low — refusing to write ants gallery index. Free up space and try again."
    );
  }
  try {
...
async function writeIndex(index: AntsGalleryIndex) {
...
export async function getAntsGallery(): Promise<AntsGalleryIndex> {
  await ensureDir();
  return readIndex();
}

export async function addImageToAntsGallery(
  image: Omit<AntsGalleryImage, "id" | "filename" | "url" | "createdAt">
): Promise<AntsGalleryImage> {
  await ensureDir();
  const id = `ants-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  ...
  try {
    ...
    throw new Error(`addImageToAntsGallery: download/write failed — ${detail}`);
  }

  const galleryImage: AntsGalleryImage = {
    ...image,
    id,
    filename,
    url: `/api/ants/gallery/file/${filename}`,
...
export async function deleteAntsGalleryImage(id: string): Promise<void> {
  const index = await readIndex();
...
export async function moveImageToAntsFolder(imageId: string, folderId: string): Promise<void> {
  const index = await readIndex();
...
export async function createAntsFolder(name: string): Promise<AntsGalleryFolder> {
  const index = await readIndex();
  const folder: AntsGalleryFolder = {
...
export async function deleteAntsFolder(id: string): Promise<void> {
  const index = await readIndex();
  index.images = index.images.map((img) =>
...
export async function renameAntsFolder(id: string, name: string): Promise<void> {
  const index = await readIndex();
