...
/**
 * Pet Tag workflow state — parallel to `useAdWorkflow.ts` with a few diffs:
 * 1. Separate localStorage key (`bugo-pet-tag-state-v1`) — independent persistence
 * 2. `selectedProductImageByRefId` — per-reference choice of which of the 2
 *    product images (product vs packaging) to use. The main flow uses a global
 *    `selectedProductImageIds` array; pet-tag needs per-reference selection.
 *
 * Reuses `ReferenceAd`/`CopyVariation`/`GenerationJob`/`Language`/`WorkflowStep`
 * types from `@/lib/types` since those are shape-compatible and don't carry
 * brand-specific logic.
 */

export interface PetTagWorkflowState {
  step: WorkflowStep;
  references: ReferenceAd[];
  /** Per-reference: which pet-tag product image (product / packaging / extra) to use. */
  selectedProductImageByRefId: Record<string, string>;
  language: Language;
  enhancedVariationMatching: boolean;
}

export type PetTagWorkflowAction =
  ...
  | { type: "SET_PRODUCT_IMAGE_FOR_REF"; refId: string; productImageId: string }
  ...
  | { type: "RESET" }
  | { type: "HYDRATE"; state: Partial<PetTagWorkflowState> };

function generateId(): string {
  return Math.random().toString(36).substring(2, 10);
}

const initialState: PetTagWorkflowState = {
  step: "upload",
  references: [],
  selectedProductImageByRefId: {},
  language: "he",
  enhancedVariationMatching: true,
};

const STORAGE_KEY = "bugo-pet-tag-state-v1";

function loadPersistedState(): Partial<PetTagWorkflowState> | null {
...
function persistState(state: PetTagWorkflowState): void {
...
function workflowReducer(state: PetTagWorkflowState, action: PetTagWorkflowAction): PetTagWorkflowState {
  switch (action.type) {
    case "SET_STEP":
    ...
    case "REMOVE_REFERENCE": {
      // Also drop the per-ref product image selection.
      const { [action.id]: _, ...remainingSelections } = state.selectedProductImageByRefId;
      void _;
      return {
        ...state,
        references: state.references.filter((r) => r.id !== action.id),
        selectedProductImageByRefId: remainingSelections,
      };
    }
    ...
    case "SET_PRODUCT_IMAGE_FOR_REF":
      return {
        ...state,
        selectedProductImageByRefId: {
          ...state.selectedProductImageByRefId,
          [action.refId]: action.productImageId,
        },
      };
...
export function usePetTagWorkflow() {
  ...
  const setProductImageForRef = useCallback(
    (refId: string, productImageId: string) =>
      dispatch({ type: "SET_PRODUCT_IMAGE_FOR_REF", refId, productImageId }),
    []
  );
