...
/**
 * Bugo Ants workflow state — parallel to useAdWorkflow.ts (the main Bugo flow),
 ...
 * because Bugo Ants has a single product (or short list of variants) used
 * consistently — same pattern as the main Bugo flow.
 *
 * Separate localStorage key (bugo-ants-state-v1) for full state isolation.
 */

export interface AntsWorkflowState {
  step: WorkflowStep;
...
export type AntsWorkflowAction =
  ...
  | { type: "RESET" }
  | { type: "HYDRATE"; state: Partial<AntsWorkflowState> };

function generateId(): string {
  return Math.random().toString(36).substring(2, 10);
}

const initialState: AntsWorkflowState = {
  step: "upload",
  references: [],
...
const STORAGE_KEY = "bugo-ants-state-v1";

function loadPersistedState(): Partial<AntsWorkflowState> | null {
...
function persistState(state: AntsWorkflowState): void {
...
function workflowReducer(state: AntsWorkflowState, action: AntsWorkflowAction): AntsWorkflowState {
  switch (action.type) {
    case "SET_STEP":
...
export function useAntsWorkflow() {
