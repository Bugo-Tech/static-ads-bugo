...
/**
 * Bugo Birds workflow state — parallel to useAdWorkflow.ts (the main Bugo flow),
 ...
 * because Bugo Birds has a single product (or short list of variants) used
 * consistently — same pattern as the main Bugo flow.
 *
 * Separate localStorage key (bugo-birds-state-v1) for full state isolation.
 */

export interface BirdsWorkflowState {
  step: WorkflowStep;
...
export type BirdsWorkflowAction =
  ...
  | { type: "RESET" }
  | { type: "HYDRATE"; state: Partial<BirdsWorkflowState> };

function generateId(): string {
  return Math.random().toString(36).substring(2, 10);
}

const initialState: BirdsWorkflowState = {
  step: "upload",
  references: [],
...
const STORAGE_KEY = "bugo-birds-state-v1";

function loadPersistedState(): Partial<BirdsWorkflowState> | null {
...
function persistState(state: BirdsWorkflowState): void {
...
function workflowReducer(state: BirdsWorkflowState, action: BirdsWorkflowAction): BirdsWorkflowState {
  switch (action.type) {
    case "SET_STEP":
...
export function useBirdsWorkflow() {
