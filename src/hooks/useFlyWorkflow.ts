...
/**
 * Bugo Fly workflow state — parallel to useAdWorkflow.ts (the main Bugo flow),
 ...
 * because Bugo Fly has a single product (or short list of variants) used
 * consistently — same pattern as the main Bugo flow.
 *
 * Separate localStorage key (bugo-fly-state-v1) for full state isolation.
 */

export interface FlyWorkflowState {
  step: WorkflowStep;
...
export type FlyWorkflowAction =
  ...
  | { type: "RESET" }
  | { type: "HYDRATE"; state: Partial<FlyWorkflowState> };

function generateId(): string {
  return Math.random().toString(36).substring(2, 10);
}

const initialState: FlyWorkflowState = {
  step: "upload",
  references: [],
...
const STORAGE_KEY = "bugo-fly-state-v1";

function loadPersistedState(): Partial<FlyWorkflowState> | null {
...
function persistState(state: FlyWorkflowState): void {
...
function workflowReducer(state: FlyWorkflowState, action: FlyWorkflowAction): FlyWorkflowState {
  switch (action.type) {
    case "SET_STEP":
...
export function useFlyWorkflow() {
